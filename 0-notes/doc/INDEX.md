...
| [architecture.md](architecture.md) | Modules, invariants (dont chemins portables Windows), dépendances entre tickets, vue de tous les projets (overview, OVERVIEW.md), démon, watcher, setup-claude, appels git (échec vs hors dépôt), hooks git, .gitignore, plugin (skill, subagent relecteur, hook de session), tests | on modifie le code de coutcouticket, on ajoute un champ ou un outil, on touche aux dépendances (blocked_by), à la vue multi-projets, au démon (démarrage, plist launchd, tâche planifiée Windows, journal), aux appels git, aux hooks git, à init, à setup-claude ou au plugin Claude Code |
