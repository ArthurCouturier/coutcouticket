...
| [architecture.md](architecture.md) | Modules, invariants, dépendances entre tickets, démon, watcher, setup-claude, hooks git, .gitignore, plugin (skill, subagent relecteur, hook de session), tests | on modifie le code de coutcouticket, on ajoute un champ ou un outil, on touche aux dépendances (blocked_by), au démon (démarrage, plist launchd, journal), aux hooks git, à init, à setup-claude ou au plugin Claude Code |
