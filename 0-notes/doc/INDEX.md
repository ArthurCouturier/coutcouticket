...
| [architecture.md](architecture.md) | Modules, invariants, dépendances entre tickets, démon, watcher, setup-claude, hooks git, .gitignore, tests | on modifie le code de coutcouticket, on ajoute un champ ou un outil, on touche aux dépendances (blocked_by), au démon (démarrage, plist launchd, journal), aux hooks git, à init ou à setup-claude |
